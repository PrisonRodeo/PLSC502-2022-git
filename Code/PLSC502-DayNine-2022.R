...
#
# Load packages (there aren't many...)
...
temp <- tempfile()
download.file("http://scdb.wustl.edu/_brickFiles/2021_01/SCDB_2021_01_caseCentered_Docket.csv.zip",temp)
df <- read.csv(unz(temp, "SCDB_2021_01_caseCentered_Docket.csv"))
unlink(temp)
...
data<-read_csv("https://raw.githubusercontent.com/PrisonRodeo/PLSC502-2022-git/master/Exercises/PLSC502-2022-ExerciseFour.csv")
...
with(data[data$Sign=="Scorpio",], 
     prop.test(sum(Active),
               nrow(data[data$Sign=="Scorpio",]),
               p=popmean,correct=FALSE))
