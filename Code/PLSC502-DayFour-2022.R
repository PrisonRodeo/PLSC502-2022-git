...
library(readr)
library(htmltab)
library(psych)   # does geometric & harmonic means + others...
library(modeest) # for modes...
library(moments)
library(stargazer)
...
print(PL[,c(2,4:10)],row.names=FALSE)

# Histogram:

pdf("PL-2022-Histogram.pdf",6,5)
par(mar=c(4,4,2,2))
...
lw<-2.5 # line width
pdf("PL-2022-Histogram2.pdf",6,5)
par(mar=c(4,4,2,2))
with(PL, hist(Points,col="grey",main="",xlab="Points Total"))
...
PL$mediandiff<- PL$Points-median(PL$Points)
PL$meandiff<- PL$Points-mean(PL$Points)
PL$absmeandiff<- abs(PL$Points-mean(PL$Points))
PL$meandiffsquared<- (PL$Points-mean(PL$Points))^2

PLtotalpoints<-sum(PL$Points)
PLtotalmediandiff<-sum(PL$mediandiff)
PLtotalmeandiff<-sum(PL$meandiff)
PLtotalabsmeandiff<-sum(PL$absmeandiff)
PLtotalmeandiffsq<-sum(PL$meandiffsquared)

# Display a couple teams:

print(PL[3,c(2,10)],row.names=FALSE)
print(PL[c(3,9),c(2,10)],row.names=FALSE)
